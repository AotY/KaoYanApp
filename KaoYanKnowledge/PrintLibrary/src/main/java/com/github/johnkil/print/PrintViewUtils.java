...
class PrintViewUtils {
    ...
    static PrintDrawable initIcon(Context context, AttributeSet attrs, boolean inEditMode) {
        ...
        if (attrs != null) {
            ...
            if (a.hasValue(R.styleable.PrintView_p_iconText)) {
                String iconText = a.getString(R.styleable.PrintView_p_iconText);
                iconBuilder.iconText(iconText);
            }

            if (a.hasValue(R.styleable.PrintView_p_iconCode)) {
                int iconCode = a.getInteger(R.styleable.PrintView_p_iconCode, 0);
                iconBuilder.iconCode(iconCode);
            }

            if (!inEditMode && a.hasValue(R.styleable.PrintView_p_iconFont)) {
                String iconFontPath = a.getString(R.styleable.PrintView_p_iconFont);
                iconBuilder.iconFont(TypefaceManager.load(context.getAssets(), iconFontPath));
            }

            if (a.hasValue(R.styleable.PrintView_p_iconColor)) {
                ColorStateList iconColor = a.getColorStateList(R.styleable.PrintView_p_iconColor);
                iconBuilder.iconColor(iconColor);
            }

            int iconSize = a.getDimensionPixelSize(R.styleable.PrintView_p_iconSize, 0);
